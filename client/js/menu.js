...
Template.menu.helpers({
  ...
  getFaves: function(itemId) {
    //load favorites
    var data = Favorites.findOne({ itemId: itemId });
    return data && data.favorites > 0 ? data.favorites : "";
  },
  ...
  if (!localCopy) {
    ...
    Meteor.call("getMenu", sYear, sMonth, sDay, day + 1, function (error, result) {
      ...
      function noMenu() {
        var courses = {
          "No Menu Available": [{
            "name": "Check Your Network Connection",
            "attributes": []
          }]
        };
...
setLocalFavorites = function(favorites) {
